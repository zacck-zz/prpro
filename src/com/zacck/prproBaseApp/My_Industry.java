...
public class My_Industry extends ListActivity {
	...
	final class dataGetter extends AsyncTask<Void, Integer, Void> {
		...
		protected void onPreExecute() {
			pd = new ProgressDialog(My_Industry.this);
			pd.setProgressStyle(ProgressDialog.STYLE_SPINNER);
			pd.setMessage("Industry News");
		...
		protected Void doInBackground(Void... arg0) {
			...
			try {
				...
				pullComm.setURI(new URI(
						"http://api2.socialmention.com/search?q=advertising+media&f=json&t[]=news&from_ts=43200"));
					...
					if (entit != null) {
						...
						if (commentAR.length() == 0) {
							...
							for (int i = 0; i < commentAR.getJSONArray("items")
								...
								times[i] = OComm.getString("timestamp");
		...
		protected void onPostExecute(Void result) {
			...
			SimpleAdapter adapter = new SimpleAdapter(My_Industry.this, commList,
