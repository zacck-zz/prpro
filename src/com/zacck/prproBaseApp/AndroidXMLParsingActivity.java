...
public class AndroidXMLParsingActivity extends ListActivity {
	...
	final class dataGetter extends AsyncTask<Void, Integer, Void> {
		...
		protected void onPreExecute() {
			pd = new ProgressDialog(AndroidXMLParsingActivity.this);
		...
		protected Void doInBackground(Void... arg0) {
			...
			myPrefs = AndroidXMLParsingActivity.this.getSharedPreferences(
					"myPrefs", MODE_WORLD_READABLE);

			// String name = myPrefs.getString("n", null);
			...
			try {
				...
				pullComm.setURI(new URI(
						"https://ajax.googleapis.com/ajax/services/search/news?v=1.0&q=unilever+blue+band&rsz=8&h1=en&ned=en_ke&scoring=d"));
				...
				if (comments.getStatusLine().getStatusCode() == 200) {
					...
					if (entit != null) {
						...
						if (commentAR.length() == 0) {
							...
							names = new String[commentAR
									.getJSONObject("responseData")
									.getJSONArray("results").length()];
							commsFromDb = new String[commentAR
									.getJSONObject("responseData")
									.getJSONArray("results").length()];
							times = new String[commentAR
									.getJSONObject("responseData")
									.getJSONArray("results").length()];
							links = new String[commentAR
									.getJSONObject("responseData")
									.getJSONArray("results").length()];
							...
							for (int i = 0; i < commentAR
								...
								commsFromDb[i] = "Source: "
										+ OComm.getString("publisher");
		...
		protected void onPostExecute(Void result) {
			...
			try {
				for (int j = 0; j < commentAR.getJSONObject("responseData")
			...
			SimpleAdapter adapter = new SimpleAdapter(
					AndroidXMLParsingActivity.this, commList,
