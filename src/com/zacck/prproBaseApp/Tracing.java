...
public class Tracing extends ListActivity {
	...
	final class dataGetter extends AsyncTask<Void, Integer, Void> {
		...
		protected void onPreExecute() {
			pd = new ProgressDialog(Tracing.this);
			pd.setProgressStyle(ProgressDialog.STYLE_SPINNER);
			pd.setMessage("Who Said IT!");
		...
		protected Void doInBackground(Void... arg0) {
			...
			try {
				...
				pullComm.setURI(new URI(
						"http://api2.socialmention.com/search?q=tbwa&f=json&t[]=all&from_ts=86400&lang=en"));
					...
					if (entit != null) {
						...
						if (commentAR.length() == 0) {
							...
							Log.v("resp", commentAR.getJSONArray("items")
									.toString());
							timeers = new String[commentAR.getJSONArray("items").length()];
							...
							for (int i = 0; i < commentAR.getJSONArray("items")
								...
								times[i] = OComm.getString("user");
								links[i] = OComm.getString("link");
								timeers[i] = OComm.getString("timestamp");
								Log.v("link", links[i]);
							}
		...
		protected void onPostExecute(Void result) {
			...
			SimpleAdapter adapter = new SimpleAdapter(Tracing.this, commList,
					R.layout.comment_row, new String[] { "name",
							"time","suggestion" }, new int[] { R.id.CommName,
							R.id.CommTime, R.id.CommText });
