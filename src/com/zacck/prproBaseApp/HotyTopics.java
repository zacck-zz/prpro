...
public class HotyTopics extends ListActivity {
	...
	final class dataGetter extends AsyncTask<Void, Integer, Void> {
		...
		protected void onPreExecute() {
			pd = new ProgressDialog(HotyTopics.this);
			pd.setProgressStyle(ProgressDialog.STYLE_SPINNER);
			pd.setMessage("Find News");
		...
		protected Void doInBackground(Void... arg0) {
			...
			try {
				...
				pullComm.setURI(new URI(
						"https://ajax.googleapis.com/ajax/services/search/news?v=1.0&rsz=8&h1=en&topic=n&ned=en_ke&userip="+IP));
				Log.v("link", pullComm.getURI().toString());
				HttpResponse comments = getComment.execute(pullComm);
				//Log.v("resp", EntityUtils.toString(comments.getEntity()));
		...
		protected void onPostExecute(Void result) {
			...
			SimpleAdapter adapter = new SimpleAdapter(HotyTopics.this, commList,
